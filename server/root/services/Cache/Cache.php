<?php namespace Root\Services\Cache;
/**
 * 
**/
...
class Cache extends Driver
{
	...
	public function __construct($config = [])
	{
		...
		if ( ! $this->is_supported($this->_adapter))
		{
			...
			{
				error_log( 'Cache adapter "'.$this->_adapter.'" and backup "'.$this->_backup_driver.'" are both unavailable. Cache is now using "Dummy" adapter.');
				$this->_adapter = 'dummy';
			}
			else
			{
				error_log( 'Cache adapter "'.$this->_adapter.'" is unavailable. Falling back to "'.$this->_backup_driver.'" backup adapter.');
