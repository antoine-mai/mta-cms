    ...
    public function render()
    {
        ...
        foreach ($options as $key => $val)
        {
            ...
            if (is_array($val))
            {
                ...
                $form .= '<optgroup label="'.\Root\Core\Common::htmlEscape($key)."\">\n";

                foreach ($val as $optgroup_key => $optgroup_val)
                {
                    $sel = in_array((string)$optgroup_key, array_map('strval', $selected)) ? ' selected="selected"' : '';
                    $form .= '<option value="'.\Root\Core\Common::htmlEscape((string)$optgroup_key).'"'.$sel.'>'
            ...
            {
                $form .= '<option value="'.\Root\Core\Common::htmlEscape($key).'"'
