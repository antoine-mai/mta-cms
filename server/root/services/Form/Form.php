<?php namespace Root\Services\Form;
...
class Form
{
    ...
    public function open($action = '', $attributes = [], $hidden = [])
    {
        ...
        if (is_array($hidden))
        {
            ...
            {
                $form .= '<input type="hidden" name="'.(string)$name.'" value="'.\Root\Core\Common::htmlEscape((string)$value).'" />'."\n";
            }
        }
    ...
    public function parse_form_attributes($attributes, $default)
    {
        ...
        foreach ($default as $key => $val)
        {
            if ($key === 'value')
            {
                $val = \Root\Core\Common::htmlEscape((string)$val);
            }
    ...
    public function set_value($field, $default = '', $html_escape = true)
    {
        ...
        return ($html_escape) ? \Root\Core\Common::htmlEscape((string)$value) : $value;
    }
