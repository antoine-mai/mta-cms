...
class Memcached extends Driver implements DriverInterface
{
    ...
    public function __construct(array &$params)
    {
        ...
        if (empty($this->config['save_path'])) {
            \Root\Core\Error::error_log( 'Session: No Memcached save path configured.');
        }
    ...
    public function open(string $savePath, string $name): bool
    {
        ...
        if (!preg_match_all('#,?([^,:]+)\:(\d{1,5})(?:\:(\d+))?#', (string)$this->config['save_path'], $matches, PREG_SET_ORDER)) {
            $this->memcached = null;
            \Root\Core\Error::error_log( 'Session: Invalid Memcached save path format: ' . $this->config['save_path']);
            return $this->failure;
        }

        foreach ($matches as $match) {
            if (in_array($match[1] . ':' . $match[2], $serverList, true)) {
                \Root\Core\Error::error_log( 'Session: Memcached server pool already has ' . $match[1] . ':' . $match[2]);
                continue;
            }

            if (!$this->memcached->addServer($match[1], (int)$match[2], isset($match[3]) ? (int)$match[3] : 0)) {
                \Root\Core\Error::error_log( 'Could not add ' . $match[1] . ':' . $match[2] . ' to Memcached server pool.');
            } else {
        ...
        if (empty($serverList)) {
            \Root\Core\Error::error_log( 'Session: Memcached server pool is empty.');
            return $this->failure;
        }
