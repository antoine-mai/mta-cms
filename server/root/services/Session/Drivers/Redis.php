...
{
    protected $redis;
    protected $keyPrefix = 'mta_cms_root_session:';
    protected $lockKey;
    ...
    public function __construct(array &$params)
    {
        ...
        if (empty($this->config['save_path'])) {
            \Root\Core\Error::error_log( 'Session: No Redis save path configured.');
            ...
            \Root\Core\Error::error_log( 'Session: Invalid Redis save path format: ' . $this->config['save_path']);
        }
    ...
    public function open(string $savePath, string $name): bool
    {
        ...
        try {
            ...
            if (!$redis->connect((string)$this->config['save_path']['host'], (int)$this->config['save_path']['port'], (float)$this->config['save_path']['timeout'])) {
                \Root\Core\Error::error_log( 'Session: Unable to connect to Redis with the configured settings.');
            } elseif (isset($this->config['save_path']['password']) && !$redis->auth((string)$this->config['save_path']['password'])) {
                \Root\Core\Error::error_log( 'Session: Unable to authenticate to Redis instance.');
            } elseif (isset($this->config['save_path']['database']) && !$redis->select((int)$this->config['save_path']['database'])) {
                \Root\Core\Error::error_log( 'Session: Unable to select Redis database with index ' . $this->config['save_path']['database']);
            } else {
                $this->redis = $redis;
                return $this->success;
            }
        } catch (RedisException $e) {
            \Root\Core\Error::error_log( 'Session: Redis connection error: ' . $e->getMessage());
        }
    ...
    public function close(): bool
    {
        if (isset($this->redis)) {
            try {
                ...
                \Root\Core\Error::error_log( 'Session: Got RedisException on close(): ' . $e->getMessage());
            }
            $this->redis = null;
            return $this->success;
        }
    ...
    protected function getLock($sessionId)
    {
        ...
        do {
            ...
            if ($ttl === -2 && !$this->redis->set($lockKey, (string)time(), ['nx', 'ex' => 300])) {
                ...
                \Root\Core\Error::error_log( 'Session: Error while trying to obtain lock for ' . $this->keyPrefix . $sessionId);
                return false;
            }
        ...
        if ($attempt === 30) {
            \Root\Core\Error::error_log( 'Session: Unable to obtain lock for ' . $this->keyPrefix . $sessionId . ' after 30 attempts, aborting.');
            return false;
        }
    ...
    protected function releaseLock()
    {
        if (isset($this->redis, $this->lockKey) && $this->lock) {
            if (!$this->redis->{$this->deleteName}((string)$this->lockKey)) {
                \Root\Core\Error::error_log( 'Session: Error while trying to free lock for ' . $this->lockKey);
                return false;
            }
            $this->lockKey = null;
            $this->lock = false;
