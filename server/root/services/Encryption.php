...
class Encryption
{
    ...
    public function __construct(array $params = [])
    {
        if (!extension_loaded('openssl')) {
            \Root\Core\trigger_error('Encryption: Unable to find an available encryption driver.', E_USER_ERROR);
        }
        ...
        if (!isset($this->key)) {
            ...
            if ($key = $config->item('encryptionKey')) {
                $this->key = $key;
            }
        }
    }
    ...
    public function initialize(array $params)
    {
        ...
        if (isset($this->cipher, $this->mode)) {
            ...
            if (!in_array($handle, openssl_get_cipher_methods(), true)) {
                $this->handle = null;
                error_log( 'Encryption: Unable to initialize OpenSSL with method ' . strtoupper((string)$handle) . '.');
            } else {
