<?php namespace Root\Services\Image;
...
class Image
{
    ...
    public function __construct($props = [])
    {
        ...
        error_log( 'Image Lib Class Initialized');
    }
    ...
    protected function loadDriver()
    {
        ...
        $class = 'Root\\Services\\Image\\Drivers\\' . ucfirst($this->image_library);
        if (class_exists($class)) {
            $this->driver = new $class($this);
            return $this->driver;
        }
        
        // Fallback to GD2 if specified driver not found
        $this->set_error('imglib_libpath_invalid'); // Use generic error
        return new Drivers\Gd2($this);
    }

    public function resize()
    {
        return $this->loadDriver()->resize();
    }

    public function crop()
    {
        return $this->loadDriver()->crop();
    }

    public function rotate()
    {
        return $this->loadDriver()->rotate();
    }

    public function watermark()
    {
        return $this->loadDriver()->watermark();
    }
    ...
    public function set_error($msg)
    {
        ...
        $mta->lang->load('imglib');
        if (is_array($msg))
        {
            foreach ($msg as $val)
            {
                $msg_text = ($mta->lang->line($val) === false) ? $val : $mta->lang->line($val);
        ...
        {
            $msg_text = ($mta->lang->line($msg) === false) ? $msg : $mta->lang->line($msg);
            $this->error_msg[] = $msg_text;
            error_log( $msg_text);
        }
    }
